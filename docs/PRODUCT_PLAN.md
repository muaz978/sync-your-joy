...
The next concrete items for M3/M5:
- Extend the real two-profile E2E coverage from the generic fixture to at least one commercial provider (Crunchyroll is the most actively maintained adapter) and wire a dedicated, separately-tuned CI job for it — SYJ-AUD-009 is not closed until provider behavior is covered too, not just the generic adapter.
- `room-service`'s local dev WebSocket server still has no global/per-IP limiter on the concurrent-room-lifetime side that the edge/production Worker already has (only its own upgrade-attempt rate limiter); lower priority since it's the local/self-hosted path, not the deployed production coordinator.
- Decide whether the still-unused `inviteToken` (generated per room but never validated by either backend) should be removed as dead code now that host-approval join covers the access-control gap it was originally meant to close, or repurposed for a future shareable-link flow.
