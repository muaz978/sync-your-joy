...
describe('playback health timing', () => {
  ...
  it('bounds a never-started player separately from ordinary playback stalls', () => {
    const deadline = playback.effectiveAtServerMs + PLAYBACK_STARTUP_TIMEOUT_MS
    expect(hasPlaybackStartupTimedOut(playback, false, deadline - 1)).toBe(false)
    expect(hasPlaybackStartupTimedOut(playback, false, deadline)).toBe(true)
